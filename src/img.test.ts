...
const generatedFiles = Object.keys(expectedGridValuesByFile).sort() as Array<
    keyof typeof expectedGridValuesByFile
>;
...
describe('pixelArtGridFromImage generated fixtures', () => {
    it.each(generatedFiles)('detects grid for %s', async (filename) => {
        ...
        expect(result.value.scaleX, `${filename}: scaleX`).toBe(
            expected.scaleX,
        );
        expect(result.value.scaleY, `${filename}: scaleY`).toBe(
            expected.scaleY,
        );
        expect(result.value.offsetX, `${filename}: offsetX`).toBe(
            expected.offsetX,
        );
        expect(result.value.offsetY, `${filename}: offsetY`).toBe(
            expected.offsetY,
        );
    });
    ...
    it('finds the correct content bounds for generated-mixed-run-length-grid-24.png', async () => {
        ...
        const filePath = join(assetDir, filename);
        const img = await read(filePath);
        const pixlerResult = await PixlerImage.createFromImage(img);

        expect(
            pixlerResult.isOk(),
            `${filename}: expected PixlerImage to create`,
        ).toBe(true);
    ...
    it('supports width, height, and getPixel for generated-mixed-run-length-grid-24.png', async () => {
        ...
        expect(
            topLeft.isOk(),
            `${filename}: expected getPixel(0, 0) to succeed`,
        ).toBe(true);
        expect(
            topRight.isOk(),
            `${filename}: expected getPixel(7, 0) to succeed`,
        ).toBe(true);
        expect(
            bottomLeft.isOk(),
            `${filename}: expected getPixel(0, 5) to succeed`,
        ).toBe(true);
        expect(
            bottomRight.isOk(),
            `${filename}: expected getPixel(7, 5) to succeed`,
        ).toBe(true);
    ...
    it('returns out-of-bounds errors for invalid PixlerImage getPixel coordinates', async () => {
        ...
        for (const [x, y] of invalidPixels) {
            ...
            expect(
                result.isErr(),
                `${filename}: expected getPixel(${x}, ${y}) to fail`,
            ).toBe(true);
